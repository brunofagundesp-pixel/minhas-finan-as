...
import { FinanceApiService, MonthDefinition, CardLaunch, CreditCard } from '../../../core/services/finance-api.service';
...
export class ConfigTabComponent implements OnInit, OnDestroy {
  ...
  /** Ano e mês selecionados para exportação (default = mês atual). */
  exportYear: number = new Date().getFullYear();
  exportMonth: number = new Date().getMonth() + 1;

  readonly exportMonthOptions: ReadonlyArray<{ value: number; label: string }> = [
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.financeApi.getCards().subscribe((cards) => {
        this.cards = cards ?? [];
      })
    );
    ...
    for (let y = currentYear - 2; y <= currentYear + 2; y += 1) {
      this.exportYearOptions.push(y);
    }
  }

  // ----------------------------------------------------------- Exportação

  exportLaunchesCsv(): void {
    const ctx = this.buildExportContext();
    this.exportService.downloadCsv(ctx);
  }

  exportLaunchesPdf(): void {
    const ctx = this.buildExportContext();
    this.exportService.openPrintablePdf(ctx);
  }

  exportDailyCsv(): void {
    const ctx = this.buildExportContext();
    this.exportService.downloadDailyCsv(ctx);
  }

  exportDailyPdf(): void {
    const ctx = this.buildExportContext();
    this.exportService.openDailyPrintablePdf(ctx);
  }

  get exportMonthLabel(): string {
    const opt = this.exportMonthOptions.find((o) => o.value === this.exportMonth);
    return `${opt?.label ?? ''} ${this.exportYear}`;
  }

  private buildExportContext() {
    return this.exportService.buildMonthExport(
      this.exportYear,
      this.exportMonth,
      this.months,
      this.cardLaunches,
